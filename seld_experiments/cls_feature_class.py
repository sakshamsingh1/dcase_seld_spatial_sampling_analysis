...
class FeatureClass:
    def __init__(self, params, is_eval=False):
        ...
        #add fibonacci tessellation parameters
        self.fib_n = params['fib_N']
        self.fib_rs = self.get_fib_rs()
        self.base_tess_rs = self.get_base_tess(params['base_tess_dist'])
        self.base_tess_mode = params['base_tess_mode']
        self.fib_tes = params['fib_tes']
        # self.pre_fib_dir = params['pre_fib_dir']
        # self.post_fib_dir = params['post_fib_dir']
    ...
    # create polar coordinates with distance of dist degrees
    def get_base_tess(self, dist):
        tess = []
        for azi in range(-180, 180+1, dist):
            for ele in range(-90, 90+1, dist):
    ...
    def extract_all_feature(self):
        ...
        print('\t\taud_dir {}\n\t\tdesc_dir {}\n\t\tfeat_dir {}'.format(
            self._aud_dir, self._desc_dir, self._feat_dir))
        arg_list = []
        for sub_folder in tqdm(os.listdir(self._aud_dir)):
            loc_aud_folder = os.path.join(self._aud_dir, sub_folder)
            for file_cnt, file_name in enumerate(os.listdir(loc_aud_folder)):
                wav_filename = '{}.wav'.format(file_name.split('.')[0])
    ...
    def extract_all_labels(self):
        ...
        for sub_folder in os.listdir(self._desc_dir):
            ...
            for file_cnt, file_name in enumerate(os.listdir(loc_desc_folder)):
                ...
                if self.fib_tes:
                    desc_file = self.convert_output_format_cartesian_to_fibonacci_tesellation(desc_file)

                if self.base_tess_mode:
                    desc_file = self.convert_output_format_cartesian_to_base_tesellation(desc_file)
    ...
    #find nearest 3d point in a list of 3d points
    def find_nearest(self, point):
        coordinates = self.fib_rs
        return coordinates[np.argmin(np.linalg.norm(coordinates - point, axis=1))]

    #def function to convert to fibonacci tesellation
    def convert_output_format_cartesian_to_fibonacci_tesellation(self, in_dict):
        out_dict = {}
        for frame_cnt in in_dict.keys():
            if frame_cnt not in out_dict:
                out_dict[frame_cnt] = []
                for tmp_val in in_dict[frame_cnt]:
                    ...
                    y = tmp_val[3]
                    z = tmp_val[4]
                    new_coord = self.find_nearest([x, y, z])
                    x_new = new_coord[0]
                    y_new = new_coord[1]
                    z_new = new_coord[2]
                    out_dict[frame_cnt].append([tmp_val[0], tmp_val[1], x_new, y_new, z_new])
        return out_dict

    # find nearest 3d point in a list of 3d points
    def find_nearest_base_tess(self, point):
        coordinates = self.base_tess_rs
        return coordinates[np.argmin(np.linalg.norm(coordinates - point, axis=1))]

    # def function to convert to fibonacci tesellation
    def convert_output_format_cartesian_to_base_tesellation(self, in_dict):
        out_dict = {}
        for frame_cnt in in_dict.keys():
            if frame_cnt not in out_dict:
                out_dict[frame_cnt] = []
                for tmp_val in in_dict[frame_cnt]:
                    ...
                    y = tmp_val[3]
                    z = tmp_val[4]
                    new_coord = self.find_nearest_base_tess([x, y, z])
                    x_new = new_coord[0]
                    y_new = new_coord[1]
                    z_new = new_coord[2]
                    out_dict[frame_cnt].append([tmp_val[0], tmp_val[1], x_new, y_new, z_new])
